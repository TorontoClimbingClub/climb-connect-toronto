...
export default function EventDetail() {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-orange-50 to-red-50 pb-20">
      <div className="w-full max-w-4xl mx-auto p-4">
        <EventHeader event={event} userJoined={userJoined} user={user} onBack={() => navigate('/events')} onJoinEvent={handleJoinEvent} onLeaveEvent={handleLeaveEvent} onEventUpdated={refreshData} />
        ...
        {/* Carpool Section */}
        <div className="mb-6">
          <CarpoolCard participants={participants} currentUserId={user?.id} currentUserParticipation={currentUserParticipation} onUpdateCarpoolStatus={handleUpdateCarpoolStatus} onAssignToDriver={handleAssignToDriver} onUpdateCarpoolPreference={handleUpdateCarpoolPreference} />
        </div>
        ...
        <div className="space-y-6">
          ...
          <EquipmentCard equipment={equipment} userEquipment={userEquipment} currentUserId={user?.id} eventId={eventId!} onRefresh={refreshData} />
        </div>
      </div>
      <Navigation />
    </div>;
